            ...
            with ui.navset_card_underline():
                with ui.nav_panel("HotSpot (Web)"):
                    ...
                    def responseTime_map():
                        ...
                        D['MeanResponse_Minutes']=round(D['MeanResponse_Minutes'],2)
                        ...
                        for index, row in BA.iterrows(): 
                            popup_text = "Address: {}<br> CBoard: {}<br> Total: {}<br> MedianMin: {}<br> MeanMin: {}<br> Min0->5: {}<br> Min5->30: {}<br> Min30->60: {}<br> Min60->360: {}<br> Min360+: {}<br> Response_Miss: {}<br> Response_No-Action: {}<br> Response_Action: {}<br> Response_Summons: {}"
                        ...
                        for index, row in BB.iterrows(): 
                            popup_text = "Address: {}<br> CBoard: {}<br> Total: {}<br> MedianMin: {}<br> MeanMin: {}<br> Min0->5: {}<br> Min5->30: {}<br> Min30->60: {}<br> Min60->360: {}<br> Min360+: {}<br> Response_Miss: {}<br> Response_No-Action: {}<br> Response_Action: {}<br> Response_Summons: {}"
                            popup_text = popup_text.format(row['incident_address'],row['cboard'] ,row['index_'],row['MedianResponse_Minutes'],row['MeanResponse_Minutes'], row["min0->5"],row["min5->30"], row["min30->60"], row["min60->360"],row["min360+"], row["Miss"],row['No-Action'], row['Action'], row['Summon'])                            folium.CircleMarker(location=(row["latitude"],row["longitude"]),
                                                radius=row['index_']/15 +3,
                                                color="#FFB52E",
                                                popup=popup_text,
                                                fill=True).add_to(NYC_map)
                ...
                with ui.nav_panel("HotSpot(Phone)"):
                    @render.text
                    def header_text3():
                        return "Click on hotspot: On bottom, adjust parameters"
                    @render.ui
                    def responseTime_map2():
                        ...
                        # generate a new map
                        NYC_map = folium.Map(location=map_location, zoom_start=zo,tiles="CartoDB positron", width="80%", height="100%")               
                        ...
                        BA=B17.query('MedianResponse_Minutes<=30')
                        for index, row in BA.iterrows(): 
                            popup_text = "Address: {}<br> Total: {}<br> MedianMin: {}<br> MeanMin: {}<br> Miss#: {}<br> No-Action#: {}<br> Action#: {}<br> Summon#: {}"
                            popup_text = popup_text.format(row['incident_address'],row['index_'],row['MedianResponse_Minutes'],row['MeanResponse_Minutes'], row["Miss"],row['No-Action'], row['Action'], row['Summon'])                            
                            folium.CircleMarker(location=(row["latitude"],row["longitude"]),
                                                radius=row['index_']/15 +3,
                                                color="#007849",
                                                popup=popup_text,
                ...
                with ui.nav_panel("City HeatMap"):
                    ui.markdown(
                        '''
